...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'admin'	=> ($user->admin == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
								'status'	=> ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
								'new'	=> ($user->new == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));
	...
	{
		return $this->ci->session->userdata('admin') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
	}
	function is_new_in($activated = TRUE)
	{
		return $this->ci->session->userdata('new') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
	}
	...
	function change_password_admin($new_pass,$user_id){
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
	...
	function chekiar_password($old_pass)
	{
		$user_id = $this->ci->session->userdata('user_id');
